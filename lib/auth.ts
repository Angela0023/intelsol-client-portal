...
export type ClientAccess = 'admin' | 'xpose' | 'tslab' | 'beeit' | 'intelsol' | 'wulf' | 'peoplefocus' | 'plantryx' | 'adsigner' | 'mountaindrop' | 'eblissai' | 'zen2fit' | 'panorate' | 'mbedtronix' | 'clevercraft' | 'stojkov' | 'birografika' | 'bmevents' | 'prtronic' | 'demo';
...
export const CLIENT_PASSWORDS: Record<ClientAccess, string> = {
  ...
  clevercraft: 'Partner.Flow.2026',
  stojkov: 'Print.Box.Serbia.2026',
  birografika: 'Label.Print.Serbia.2026',
...
export function getAllowedClients(password: string): ClientAccess[] {
  ...
  if (checkAdminAccess(password)) {
    return ['admin', 'xpose', 'tslab', 'beeit', 'intelsol', 'wulf', 'peoplefocus', 'plantryx', 'adsigner', 'mountaindrop', 'eblissai', 'zen2fit', 'panorate', 'mbedtronix', 'clevercraft', 'stojkov', 'birografika', 'bmevents', 'prtronic', 'demo'];
  }
